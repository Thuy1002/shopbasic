                       ...
                       .done(function (data, status) {
                           if (status == 'success') {
                               $.each(data, function (index, item) {
                                   // console.log(index);
                                   _strHtml += '<tr>';
                                   // ma_hv: "D180002", ngay_vao_lop:  status_hoc_tap: 2, lich_su_status: null}

                                   _strHtml += '<td>' + item.ma_lop + '</td><td>' + item.ngay_bat_dau + '</td>' + '</td><td>' + item.status + '</td>';
                                   _strHtml += '<tr>';
                               });
                       ...
                       if(status == 'success'){
                           if(data.status !=1 ){
                               ...
                               $.each(data.list,function (index,item) {
                                   ...
                                   var __trangthai = '';
                                   $.each(data.status_hoc,function (_idex,itemTT) {
                                       if(_idex ==item.status_hoc)
                                       {
                                           __trangthai = itemTT;
                                           return;
                                       }
           ...
           $('.cham-soc-hv').click(function () {
               ...
               if(appHocVien.trangThaiChamSoc.length>0){
                   $.each(appHocVien.trangThaiChamSoc,function (index,item) {
                       _strHtml += '<option value="'+item.id+'">' + item.ten_status + '</option>';
                   });
               }
       ...
       appHocVien.setTrangThaiHocTap = function () {
           ...
           $('.set-trang-thai-hoc-tap').click(function () {
               ...
               _strHtml += '<select id="tt_ht" class="form-control col-sm-6 col-xs-6"  style="margin-bottom: 10px">' +
                ...
                console.log(status_hoc_tap );

               if(typeof status_hoc_tap != 'undefined'){
                   $.each(status_hoc_tap,function (index,item) {
...
// console.log(ngay_thi);
                                    i++;
                                    div +='<tr class="'+(notShow.includes(item.status_hoc)?'active':'')+'"><td>'+i+'</td>';
                                    div +='<td>'+item.mahv+'</td>';
                                    div +='<td>'+item.ho_dem + ' ' + item.ten+'</td>';
                                    const ns = item.ngay_sinh!=null?item.ngay_sinh.split('-').reverse().join('/'):'';
                                    div +='<td>'+ns+'</td>';
                                    ...
                                    div +='<td><input '+(notShow.includes(item.status_hoc)?'disabled':'')+' type="checkbox" name="chk_hv[]" value="'+item.id_lmhv+'" class="chk_hv" '+checked+'></td>';
        ...
        appHuongNghiep.run = function () {
                            ...
                            if(data.status !=1 ){
                                ...
                                if(appHuongNghiep.update.length>0){
                                    ...
                                    $.each(appHuongNghiep.update,function (index,item) {
                                        ...
                                        _strHtml += '<tr><td>'+data.viec_lam[item.status_viec_lam]+'</td><td>'+item.thong_tin_cong_viec+'</td><td>'+nhucau+'</td><td>'+item.ghi_chu+'</td><td>'+ngaygui+'</td></tr>';
                                    });
                                    _strHtml += '</table>';
                                }
            ...
            $('.btnUpdateV').click(function () {
                ...
                $.post( '/apps/ajax-update-trang-thai-hoc-vien/'+appHuongNghiep.idhvn,
                  ...
                  ttvl:$('#status_viec_lam_'+ appHuongNghiep.idhvn).val()})
                    .done(function(data, status) {
                        if(status == 'success'){
                            console.log(data);
                            if(data.status !=1 ){
                            ...
                            if(data.status !=1 ){
                                ...
                                if(appDoAn.hocVien.length>0){
                                    ...
                                    $.each(appDoAn.hocVien,function (index,item) {
                                        ...
                                        _strHtml += '<tr><td>'+item.id+'</td><td>'+item.mahv+'</td><td>'+item.ho_dem+' '+item.ten+'</td><td class="heso">'+heso+'</td><td>'+diem+'</td><td>'+data.status[item.status]+'</td></tr>';
                                    });
                                    _strHtml += '</table>';
                                }
                           ...
                           if (status == 'success') {
                               if(data.status !=1 ){
                                   ...
                                   if(appDoAn.hocVien.length>0){
                                       ...
                                       $('#cs_msg span').remove();
                                       $.each(appDoAn.hocVien,function (index,item) {
                                           _strHtml += '<tr><td>'+item.id+'</td><td>'+item.mahv+'</td><td>'+item.ho_dem+' '+item.ten+'</td><td><select name="status_'+item.id+'" class="form-control" id="">';
                                           $.each(appDoAn.trangthai,function(k,v){
                                               var selected = item.status == k?'selected':'';
                    ...
                    .done(function (data) {
                        ...
                        if(appTuyenSinh.listChamSoc.length>0){
                            ...
                            $.each(appTuyenSinh.listChamSoc, function (index, item) {
                                _strHtml += "<tr><td>"+item.ten_status+"</td><td>"+item.noi_dung+"</td><td>"+appTuyenSinh.formatDateTime(item.c_time)+"</td></tr>"
                            })
